...
public:
    ...
    void load_image_png(const char *Path, string name);                         // 加载png图片
    void printJPEGDecompressStructMembers(const jpeg_decompress_struct &cinfo); // 打印jpeg解码器信息

    bool is_in_range(int x, int y); // 判断坐标是否在屏幕范围内
    bool is_in_range(Vector2 pos);  // 判断坐标是否在屏幕范围内

    bool is_han(wchar_t c);                                     // 判断是否是汉字
    int get_utf8_len_from_first_byte(unsigned char first_byte); // 获取utf8编码的字节数
public:
    ...
    void remove_layer(string name); // 删除图层
    void clear_layer(int index);    // 清空图层
    void clear_layer(string name);  // 清空图层
    void show_layer(int index);     // 显示图层
    void show_layer(string name);   // 显示图层
    void hide_layer(int index);     // 隐藏图层
    void hide_layer(string name);   // 隐藏图层
    ...
    void Print_Text(int layer_index, Vector2 start_pos, Vector2 end_pos, string text,
                    float scale = 1.0, pixel color = 0xFF000000, Vector2 LC_adjust = Vector2(0, 0)); // 打印文本到图层
    void Print_Text(string layer_name, Vector2 start_pos, string text,
                    float scale = 1.0, pixel color = 0xFF000000, Vector2 LC_adjust = Vector2(0, 0)); // 打印文本到图层
    void Print_Text(string layer_name, Vector2 start_pos, Vector2 end_pos, string text,
                    float scale = 1.0, pixel color = 0xFF000000, Vector2 LC_adjust = Vector2(0, 0)); // 打印文本到图层

    void draw_image(int layer_index, Vector2 loc, int image_index, Vector2_f pivot);         // 绘制图片到图层
    void draw_image(string layer_name, Vector2 loc, string image_name, Vector2_f pivot);     // 绘制图片到图层
    void draw_image(string layer_name, Vector2 loc, int image_index, Vector2_f pivot);       // 绘制图片到图层
